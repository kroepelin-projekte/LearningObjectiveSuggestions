...
class CourseConfigProvider
{
    ...
    public function getWeightRough(LearningObjective $learning_objective, StudyProgram $study_program): string
    {
        return (string) $this->get('weight_rough_' . $learning_objective->getId() . '_' . $study_program->getId());
    }
    public function getWeightFine(LearningObjective $learning_objective): string
    {
        return (string) $this->get('weight_fine_' . $learning_objective->getId());
    }
